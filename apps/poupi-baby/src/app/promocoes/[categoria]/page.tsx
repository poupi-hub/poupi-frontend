...
export default async function PromoçõesPage({ params }: Props) {
  const { categoria } = await params;
  const data = await fetchCategoryData(categoria);
  if (!data) notFound();
            ...
            {allProducts.map((p) => {
              ...
              return (
                <a
                  key={p.id}
                  href={`/produto/${p.slug}`}
                  className={`rounded-lg border bg-white p-4 shadow-sm transition hover:border-[#cdb8ef] ${hasDiscount ? 'border-[#5B4CF0]' : 'border-[#E4E7F2]'}`}
                >
